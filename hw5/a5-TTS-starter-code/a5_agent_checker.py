...
if len(sys.argv) < 2:
  ...
  print("  for example:")
  print("python3 a5_agent_checker.py daw1230_TTS_agent")
  exit(0)
...
try:
	state = TTS_State.TTS_State(board)
	results = Player.parameterized_minimax(current_state=state, max_ply=2, alpha_beta=True, use_custom_static_eval_function=False)
	assert(results["CURRENT_STATE_STATIC_VAL"] == 1)
##	print(results["N_STATES_EXPANDED"])
	assert(results["N_STATES_EXPANDED"] == 4)
##	print(results["N_STATIC_EVALS"])
